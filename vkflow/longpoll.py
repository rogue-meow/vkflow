...
from vkflow.base.event_factories import BaseLongPoll, EventsCallback
from vkflow.event import GroupEvent, UserEvent
...
class GroupLongPoll(BaseLongPoll):
    ...
    async def _setup(self) -> None:
        ...
        new_lp_settings = await self.api.method("groups.getLongPollServer", group_id=self._group_id)

        self._server_url = new_lp_settings.pop("server")
        self._requests_query_params = dict(act="a_check", wait=self._wait, **new_lp_settings)
    ...
    async def _define_group_id(self) -> None:
        if self._group_id is None:
            ...
            if token_owner != TokenOwner.GROUP:
                raise ValueError("Can't use `GroupLongPoll` with user token without `group_id`")
...
class UserLongPoll(BaseLongPoll):
    ...
    async def _setup(self) -> None:
        new_lp_settings = await self.api.method("messages.getLongPollServer", lp_version=self._version)
